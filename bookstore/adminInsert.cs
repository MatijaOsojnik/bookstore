...
namespace bookstore
{
    public partial class adminInsert : Form
    {
        ...
        {
            DBConnect db = new DBConnect();
            MySqlCommand command = new MySqlCommand("INSERT INTO `knjige`(`naslov`, `opis`, `strani`, `datum_izdaje`, `avtor_id`, `zalozba_id`) VALUES (@naslov, @opis, @strani, @datum, (SELECT id FROM avtorji WHERE ime = @avtor),  (SELECT id FROM zalozbe WHERE ime = @zalozba))", db.GetConnection());
