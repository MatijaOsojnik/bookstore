...
namespace bookstore
{
    public partial class adminUpdate : Form
    {
        ...
        {
            DBConnect db = new DBConnect();
            MySqlCommand command = new MySqlCommand("UPDATE `knjige` SET `naslov`=@naslov, `opis`=@opis, `strani`=@strani, `datum_izdaje`=@datum, `avtor_id`=(SELECT id FROM avtorji WHERE ime = @avtor), `zalozba_id`=(SELECT id FROM zalozbe WHERE ime = @zalozba) WHERE `id` = @id ;", db.GetConnection());
        ...
        private void adminUpdate_Load(object sender, EventArgs e)
        {
            ...
            while (reader1.Read())
            {
                avtorComboBox.Items.Add(reader1.GetString("ime"));
